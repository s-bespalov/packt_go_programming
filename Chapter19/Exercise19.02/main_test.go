...
func NewDatadase(conn *sql.DB) *Database {
...
func (d *Database) GetRecordByID(ctx context.Context, id int) (Record, error) {
	...
	row := d.conn.QueryRowContext(ctx,
		"SELECT id, name, value FROM record WHERE id = $1", id)
...
func TestDatabaseIntegration(t *testing.T) {
	...
	mock.ExpectQuery("SELECT id, name, value FROM record").
		WillReturnRows(rows)
	dbInstance := NewDatadase(db)
